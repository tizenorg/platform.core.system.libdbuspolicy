										 ...
										 const char*& privilege)
{
	...
	return _ldp_xml_parser::NO_DECISION;
}

_ldp_xml_parser::NaivePolicyDb& _ldp_xml_parser::NaivePolicyChecker::getPolicyDb(bool type) {
	if (type)
		return m_session_db;
	else
		return m_system_db;
...
bool _ldp_xml_parser::NaivePolicyChecker::parseDecision(Decision decision,
														uid_t uid,
														const char* label,
														const char* privilege) {
...
_ldp_xml_parser::NaivePolicyChecker::~NaivePolicyChecker() {
	delete m_adapter;
}

bool _ldp_xml_parser::NaivePolicyChecker::checkItem(bool bus_type, uid_t uid, gid_t gid, const char* label, const Item& item) {
	...
	if (ret == NO_DECISION) {
		curr_policy = policy_db.getPolicy(type, CONTEXT, PolicyTypeValue(MANDATORY));
		if (curr_policy)
			ret = checkPolicy(*curr_policy, item, privilege);
	}

	if (ret == NO_DECISION) {
		curr_policy = policy_db.getPolicy(type, USER, PolicyTypeValue(uid));
		if (ret == NO_DECISION && curr_policy)
			ret = checkPolicy(*curr_policy, item, privilege);
	}

	if (ret == NO_DECISION) {
		curr_policy = policy_db.getPolicy(type, GROUP, PolicyTypeValue(gid));
		if (ret == NO_DECISION && curr_policy)
			ret = checkPolicy(*curr_policy, item, privilege);
	}

	if (ret == NO_DECISION) {
		curr_policy = policy_db.getPolicy(type, CONTEXT, PolicyTypeValue(DEFAULT));
		if (ret == NO_DECISION && curr_policy)
			ret = checkPolicy(*curr_policy, item, privilege);
	}
...
bool _ldp_xml_parser::NaivePolicyChecker::check(bool bus_type,
														uid_t uid,
														gid_t gid,
